...
class PutCertificatesId extends BaseEndpoint
{
    protected $id;
     ...
     * }
     */
    public function __construct(string $id, ?CertificateUpdate $requestBody = null, array $headerParameters = [])
    {
        $this->id = $id;
    ...
    {
        if ($this->body instanceof CertificateUpdate) {
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null)
    {
        ...
        if (is_null($contentType) === false && (200 === $status && str_contains($contentType, 'application/json'))) {
            return $serializer->deserialize($body, Certificate::class, 'json');
        }
    }
